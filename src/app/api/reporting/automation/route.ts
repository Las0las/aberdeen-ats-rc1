...
import { fetchAutomationEffectiveness } from "@/lib/reporting/queries";

const log = createLogger("api:reporting:automation");
...
export async function GET(request: NextRequest) {
  try {
    ...
    const result = await fetchAutomationEffectiveness(supabase, parsed.data.sinceDays);
    return NextResponse.json(result);
  } catch (err: unknown) {
    const message = err instanceof Error ? err.message : "Internal error";
    log.error("automation handler failed", message);
