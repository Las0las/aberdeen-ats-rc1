import type { SupabaseClient } from "@supabase/supabase-js";
import type { AssignmentsRow } from "@/contracts/db/tables";

// List: exclude notes (can be long text)
const LIST_FIELDS = [
  "id",
  "organization_id",
  "candidate_id",
  "job_id",
  "application_id",
  "state",
  "start_date",
  "current_end_date",
  "client_name",
  "engagement_type",
  "created_by",
  "created_at",
  "updated_at",
].join(",");

// Detail: all scalar fields
const DETAIL_FIELDS = [
  LIST_FIELDS,
  "workspace_id",
  "notes",
].join(",");

// candidate_id is NOT NULL so join always resolves
const CANDIDATE_JOIN = "candidates(id,first_name,last_name,current_title,email)";
// job_id is NOT NULL so join always resolves
const JOB_JOIN = "jobs(id,title,department,location)";

export interface AssignmentWithRelations extends AssignmentsRow {
  candidates: {
    id: string;
    first_name: string | null;
    last_name: string | null;
    current_title: string | null;
    email: string | null;
  } | null;
  jobs: {
    id: string;
    title: string | null;
    department: string | null;
    location: string | null;
  } | null;
}

async function resolveOrgId(supabase: SupabaseClient): Promise<string> {
  const { data, error } = await supabase.rpc("current_org_id");
  if (error || !data) throw new Error("Unable to resolve organization");
  return data as string;
}

export async function listAssignments(
  supabase: SupabaseClient,
  opts: {
    state?: string;
    candidate_id?: string;
    job_id?: string;
    search?: string;
    page: number;
    limit: number;
  },
) {
  const orgId = await resolveOrgId(supabase);
  const from = (opts.page - 1) * opts.limit;
  const to = from + opts.limit - 1;

  let query = supabase
    .from("assignments")
    .select(`${LIST_FIELDS}, ${CANDIDATE_JOIN}, ${JOB_JOIN}`, { count: "exact" })
    .eq("organization_id", orgId)
    .order("updated_at", { ascending: false })
    .range(from, to);

  if (opts.state) query = query.eq("state", opts.state);
  if (opts.candidate_id) query = query.eq("candidate_id", opts.candidate_id);
  if (opts.job_id) query = query.eq("job_id", opts.job_id);

  const { data, error, count } = await query;
  if (error) throw error;

  let results = (data ?? []) as unknown as AssignmentWithRelations[];

  // Client-side search across candidate name, job title, client_name
  if (opts.search) {
    const term = opts.search.toLowerCase();
    results = results.filter((a) => {
      const cName = [a.candidates?.first_name, a.candidates?.last_name]
        .filter(Boolean)
        .join(" ")
        .toLowerCase();
      const jTitle = a.jobs?.title?.toLowerCase() ?? "";
      const client = a.client_name?.toLowerCase() ?? "";
      return cName.includes(term) || jTitle.includes(term) || client.includes(term);
    });
  }

  return { assignments: results, total: count ?? 0 };
}

export async function getAssignment(
  supabase: SupabaseClient,
  id: string,
): Promise<AssignmentWithRelations> {
  const orgId = await resolveOrgId(supabase);
  const { data, error } = await supabase
    .from("assignments")
    .select(`${DETAIL_FIELDS}, ${CANDIDATE_JOIN}, ${JOB_JOIN}`)
    .eq("organization_id", orgId)
    .eq("id", id)
    .single();
  if (error) throw error;
  return data as unknown as AssignmentWithRelations;
}
